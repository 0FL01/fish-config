...
- Установщик принудительно добавляет в `~/.zshrc` флаги `DISABLE_AUTO_UPDATE="true"` и `DISABLE_UPDATE_PROMPT="true"`, чтобы отключить автообновления и напоминания.
- Чтобы вернуть проверку обновлений, удалите эти строки или выставьте значения в `false` и перезапустите zsh (`exec zsh`).
